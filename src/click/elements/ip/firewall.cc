...
#define TIME_WAIT_STATE 7    //  6 7 主动关闭
...
Firewall::~Firewall() 
{
	...
	click_chatter(connectStateSa.c_str());
	FILE *connectStateFp = NULL;
	...
	click_chatter(passStateSa.c_str());
	FILE *passStateFp = NULL;
...
int Firewall::listenSel(uint8_t th_flags, IPFlowID flowid){
	int res;
	click_chatter("listenSel %d %d", th_flags, TH_SYN|TH_ACK);
	switch(th_flags){
		case TH_SYN|TH_ACK : {
...
int Firewall::synRcvdSel(uint8_t th_flags, IPFlowID flowid){
	...
	switch(th_flags){
		case TH_ACK : {
			connectMap.insert(flowid.unparse(), ESTABLISHED_STATE);   //验证ack包
			res = 1;
			break;
		}
		case TH_PUSH|TH_ACK : {
...
void Firewall::push(int i, Packet *p_in)
{
	...
	click_chatter("passState = %d", passState);
	switch(passState){
		case 1 : output(0).push(p); return;
		case 2 : break;
	}
	click_chatter("enter sessioncheck!!!");
	int op = -1;
	switch(connectState){
	...
	click_chatter("hashmap.size = %d", connectMap.size());
	}
	output(n).push(p);
}
